...
// Updated: DOMContentLoaded + null-safety guards
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // defensive fallbacks (if something unexpected in HTML)
  if (!assetsTableBody) {
    console.warn('assets.js: missing #assetsTable tbody element');
  }
  if (!paginationRow) {
    console.warn('assets.js: missing #paginationRow element; pagination UI will be disabled');
  }
  ...
  function renderPaginationControls(total, totalPages) {
    if (!paginationRow) return; // nothing to render into

    // remove all children except the pagerInfo (if present)
    // If pagerInfo is not the first child, clear entire container
    if (pagerInfo && paginationRow.contains(pagerInfo)) {
      // remove children after pagerInfo
      while (paginationRow.children.length > 1) paginationRow.removeChild(paginationRow.lastChild);
    } else {
      // remove all children and re-add pagerInfo placeholder
      while (paginationRow.firstChild) paginationRow.removeChild(paginationRow.firstChild);
      if (pagerInfo) paginationRow.appendChild(pagerInfo);
    }
